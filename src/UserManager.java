...
class Admin extends User implements EmployeeManagement, ClientManagement {
    ...
    private void newClient() {//добави тези два метода в нов клас
        Scanner scanner = new Scanner(System.in);
    ...
    private Date getValidContractExpirationDate() {//валидации за дата
        Scanner scanner = new Scanner(System.in);
    ...
    private boolean doesEmployeeNameExists(String employeeName, File reportFile) {
        try (Scanner fileScanner = new Scanner(reportFile)) {
            while (fileScanner.hasNextLine()) {
                ...
                if (parts.length == 2) {
                    String storedUsername = parts[0].trim().toLowerCase(); // Convert to lowercase
